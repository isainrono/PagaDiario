# Agente Frontend — Monopoly Companion
...
interface GameStore {
  ...
  // Propiedades
  propiedades: Propiedad[];             // estado de las 28 propiedades comprables
...
socket.on('tablero:movimiento', (data) => {
  ...
  // mostrar modal de acción si es el jugador activo
});
```

Para la lista completa de eventos ver: catálogo de eventos (Agente Real-time).
...
2. El countdown de subastas debe ser visible en cualquier tab cuando hay una activa (usar un banner persistente fuera del Tab Bar, no solo en el tab de jugadores).
...
- No inventas endpoints ni eventos que no estén en los contratos. Si necesitas algo nuevo, lo coordinas con el Agente Arquitecto.
